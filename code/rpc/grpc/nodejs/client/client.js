...
// 设置ssl
// scrips下的文件用来生成ssl文件给server和client
// let unsafCreds = grpc.credentials.createSsl(
...
function callGreetings() {
  ...
  // 创建server
  // var client = new service.GreetServiceClient("localhost:50051", grpc.credentials.creteInsecure());
  // 指定需要连接的server的地址
  ...
  // 创建protobuf中的greeting
  var greeting = new greets.Greeting();
  greeting.setFirstName("Jacob");
  greeting.setLastName("Huang");
  // 向request中传入greeting对象
...
// nodejs 并没有实现原生的deadline，所以需要自己写函数
function getRPCDeadline(rpcType) {
  timeAllowed = 5000;

  switch (rpcType) {
    case 1:
      timeAllowed = 1000; // millionseconds
      break;
...
function doErrorCall() {
  // deadline是什么？
  // deadline allow gRPC clients to specify how long they are willing to
  // wait for an RPC to complete before the RPC is terminated with the error DEADLINE_EXCEEDED
